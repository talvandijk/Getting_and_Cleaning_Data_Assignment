...
Steps in reading and cleaning data
----------------------------------

The training and test data were read and each merged with activity codes. The training and test data were merged. The activity labels were added to the merged dataset according to their responding code. Then the relevant variables were set: the id's for the test subjects and activity id's, and the means and standard deviations of all different measurements. Variable names were changed and given slightly more intuitive names. This resulted in a tidy dataset. On the basis of this tidy dataset a second tidy dataset was created by aggregating measurements per subject and per activity performed by each subject.
